...
def get_vehiculos(page: int = 1, page_size: int = 10, db: Session = Depends(get_db)):
    return crud_vehiculo.get_pagination(db, page=page, size=page_size)

@router.get("/{veh_id}", response_model=VehicleOut)
def get_vehiculo(veh_id: int, db: Session = Depends(get_db)):
...
def get_vehiculos_by_cliente(cli_id: int, db: Session = Depends(get_db)):
    return crud_vehiculo.get_by_client(db, cli_id)

@router.post("/", response_model=VehicleOut)
def create_vehiculo(vehiculo_in: VehicleCreate, db: Session = Depends(get_db)):
...
def update_vehiculo(veh_id: int, vehiculo_in: VehicleUpdate, db: Session = Depends(get_db)):
    vehiculo = crud_vehiculo.get(db, veh_id)
    if not vehiculo:
        raise HTTPException(status_code=404, detail="Vehiculo no encontrado")
...
def delete_vehiculo(veh_id: int, db: Session = Depends(get_db)):
